    ...
    do
        ...
        if f then
            ...
            do
                ...
                bitrate, bu = line:match("[%d%.]+-[%d%.]+%s+sec%s+[%d%.]+ [KM]Bytes%s+([%d%.]+) ([MK])bits/sec%s+receiver")
                if bitrate then
                    bitrate = tonumber(bitrate) / (bu == "K" and 1024 or 1)
                    break
                end
                bitrate, bu = line:match("[%d%.]+-[%d%.]+%s+sec%s+[%d%.]+ [KM]Bytes%s+([%d%.]+) ([MK])bits/sec%s+[%d%.]+ ms%s+%d+/%d+ %(([%d%.]+)%%%)%s+receiver")
                if bitrate then
                    bitrate = tonumber(bitrate) / (bu == "K" and 1024 or 1)
                    break
                end
    ...
    do
        if c.name == "iperf3" and (c.frequency == "hourly" or (c.frequency == "daily" and opt.is_daily)) then
            ...
            if hostname and M.is_peer(hostname) then
                ...
                metrics['node_periodic_iperf3_rx_bitrate{hostname="' .. hostname .. '",protocol="' .. protocol .. '"}'] = rx
                metrics['node_periodic_iperf3_tx_bitrate{hostname="' .. hostname .. '",protocol="' .. protocol .. '"}'] = tx
            end
        end
    end
